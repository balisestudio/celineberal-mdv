...
const deleteExistingLots = async (
	...
	log("info", "Suppression des anciens lots", {
		auctionId,
	...
	log("info", "Tous les anciens lots supprimés", {
		count: existingLots.totalDocs,
	});
};
...
const processLotContent = async (
	...
	if (options.optimizeContent) {
		try {
			log("info", "AI optimisation - début", { lotNumber });
			...
			log("info", "AI optimisation - terminé", {
				lotNumber,
				title: frTitle,
			...
			log("warn", "AI optimisation échouée - fallback données brutes", {
				lotNumber,
	...
	if (options.translateContent) {
		try {
			log("info", "AI traduction - début", { lotNumber, locale: "en" });
			...
			log("info", "AI traduction - terminé", {
				lotNumber,
				locale: "en",
				title: enTitle,
			});
		} catch (error) {
			log("warn", "AI traduction échouée - pas de locale EN", {
				lotNumber,
				error: error instanceof Error ? error.message : String(error),
			});
		}
...
const uploadLotImages = async (
	...
	for (let i = 0; i < sorted.length; i++) {
		...
		log("info", "Upload image vers Payload", {
			lotNumber,
			filename,
			alt,
			size: buffer.length,
...
const prepareSingleLot = async (
	...
	try {
		...
		log("error", "Préparation lot échouée", {
			lotNumber,
			error: errorMessage,
		});
		return null;
...
const cleanupOrphanMedia = async (mediaIds: number[]) => {
	...
	try {
		...
		log("info", "Médias orphelins supprimés", { count: mediaIds.length });
	} catch (cleanupError) {
		log("warn", "Échec nettoyage médias orphelins", {
			error:
...
export const importLotsFunction = inngest.createFunction(
	{
		id: "import-lots",
		triggers: { event: "auction/lots.import.requested" },
		onFailure: async ({ error, event }) => {
			const originalEvent = event.data.event as {
				data?: ImportLotsPayload;
			};
			const auction = originalEvent?.data?.auction;
			if (!auction) return;

			const errorMessage =
				error instanceof Error ? error.message : String(error);
			log("error", "Hook onFailure - nettoyage triggerId après échec", {
			...
			try {
				...
				log("error", "Hook onFailure - impossible de nettoyer triggerId", {
					auctionId: auction.id,
					error:
						cleanupError instanceof Error
							? cleanupError.message
	...
	async ({ event, step }) => {
		...
		try {
			...
			log("error", "Erreur lors de la suppression des lots - rollback", {
				auctionId: auction.id,
				error: error instanceof Error ? error.message : String(error),
			});
		...
		try {
			...
			if (preparedFailed > 0) {
				log("warn", "Lots non préparés (ignorés)", {
					auctionId: auction.id,
					count: preparedFailed,
				});
			}

			log("info", "Hook onSuccess - nettoyage triggerId", {
				auctionId: auction.id,
			});
			await clearTriggerId(auction.id);
		} catch (error) {
			log("error", "Erreur pendant l'écriture des lots - rollback", {
				auctionId: auction.id,
				error: error instanceof Error ? error.message : String(error),
			});
